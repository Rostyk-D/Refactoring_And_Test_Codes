...
from Project_v2.repository.sqlite_repository import (
    SQLiteBookRepository,
    SQLiteUserRepository,
    SQLiteLoanRepository,
)
from Project_v2.repository.factory import RepositoryFactory, RepoBundle
...
from Project_v2.service.library_service import LibraryService
from Project_v2.Client import LibraryGUI
...
class TestLibraryGUI(unittest.TestCase):
    def setUp(self):
        patch('Project_v2.Client.LibraryGUI._build_books_tab', lambda s: None).start()
        patch('Project_v2.Client.LibraryGUI._build_users_tab', lambda s: None).start()
        self.mod = __import__('Project_v2.Client', fromlist=['service'])
    ...
    def test_delete_book_popup_calls_remove(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', return_value=isbn_entry), \
    ...
    def test_edit_book_popup_not_found(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', return_value=ent), \
    ...
    def test_edit_book_popup_found(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', return_value=MagicMock(get=MagicMock(return_value="EDITISBN"))), \
...
class TestLibraryGUIPopupMethods(unittest.TestCase):
    ...
    def test_search_books_popup_no_results_and_with_results(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', side_effect=entry_side_effect):
        ...
        with patch('Project_v2.Client.ttk.Entry', side_effect=lambda parent: mocks.pop(0)):
            ...
            def fake_button2(parent, text, command, **kwargs):
                ...
                if text == "Пошук":
                    command()
                return btn
            with patch('Project_v2.Client.ttk.Button', side_effect=fake_button2):
                self.app.search_books_popup()
    ...
    def test_add_book_popup_success_and_failure(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', side_effect=lambda parent: mocks.pop(0)), \
             patch('Project_v2.Client.random.randint', return_value=1), \
             patch('Project_v2.Client.messagebox.showinfo') as mock_info, \
        ...
        with patch('Project_v2.Client.ttk.Entry', side_effect=lambda parent: mocks_fail.pop(0)), \
             patch.object(self.mod.service, 'add_book', side_effect=Exception("Fail")), \
             patch('Project_v2.Client.messagebox.showerror') as mock_error2, \
             patch('Project_v2.Client.ttk.Button', side_effect=lambda parent, text, command, **kw: command() or MagicMock()):
            self.app.add_book_popup()
    ...
    def test_add_user_popup_success_and_failure(self):
        ...
        with patch('Project_v2.Client.ttk.Entry', side_effect=lambda parent: mocks.pop(0)), \
             patch('Project_v2.Client.uuid.uuid4', return_value=MagicMock(__str__=MagicMock(return_value="fixeduuid"))), \
    ...
    def test_issue_book_popup_success_and_failure(self):
        ...
        with patch('Project_v2.Client.tk.Entry', side_effect=lambda parent: mocks.pop(0)), \
            ...
            def fake_button(parent, text, command, **kwargs):
                ...
                if text == "Підтвердити":
                    command()
                return btn
            with patch('Project_v2.Client.tk.Button', side_effect=fake_button):
        ...
        with patch('Project_v2.Client.tk.Entry', side_effect=lambda parent: mocks_fail.pop(0)), \
            ...
            def fake_button2(parent, text, command, **kwargs):
                ...
                if text == "Підтвердити":
                    command()
                return btn
            with patch('Project_v2.Client.tk.Button', side_effect=fake_button2):
                self.app.issue_book_popup()
...
class TestLibraryGUIStructure(unittest.TestCase):
    def setUp(self):
        patch('Project_v2.Client.service', MagicMock()).start()
        self.app = LibraryGUI()
